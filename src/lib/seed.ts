...
export function seedDatabase() {
  ...
  const existing = db.prepare('SELECT COUNT(*) as count FROM hackathons').get() as { count: number };
  if (existing.count > 0) {
  ...
  const now = new Date();
  const startDate = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000); // Started 1 week ago
  const endDate = new Date(now.getTime() + 21 * 24 * 60 * 60 * 1000); // Ends in 3 weeks

  db.prepare(`
    INSERT INTO hackathons (id, name, description, start_date, end_date, tracks, prizes, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    hackathonId,
    'Solana AI Hackathon',
  ...
  for (const agent of agents) {
    ...
    agentIds.push(id);
    db.prepare(`
      INSERT INTO agents (id, name, description, api_key, owner_name, claim_code, karma, created_at, last_active)
      VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    `).run(id, agent.name, agent.description, generateApiKey(), agent.owner_name, generateClaimCode(), Math.floor(Math.random() * 100));
  }
  ...
  for (const team of teams) {
    ...
    db.prepare(`
      INSERT INTO teams (id, name, hackathon_id, invite_code, created_by, created_at)
      VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    `).run(teamId, team.name, hackathonId, inviteCode, agentIds[team.agentIndices[0]]);

    for (let i = 0; i < team.agentIndices.length; i++) {
      db.prepare(`
        INSERT INTO team_members (team_id, agent_id, role, joined_at)
        VALUES (?, ?, ?, CURRENT_TIMESTAMP)
      `).run(teamId, agentIds[team.agentIndices[i]], i === 0 ? 'leader' : 'member');
    }
  }
  ...
  for (const project of projects) {
    ...
    db.prepare(`
      INSERT INTO projects (id, name, description, track, repo_url, demo_url, tech_stack, team_id, hackathon_id, status, votes, submitted_at, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    `).run(
      projectId,
      project.name,
    ...
    // Add an update for submitted projects
    if (project.status === 'submitted') {
      db.prepare(`
        INSERT INTO updates (id, project_id, content, week_number, created_at)
        VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
      `).run(
        uuidv4(),
        projectId,
